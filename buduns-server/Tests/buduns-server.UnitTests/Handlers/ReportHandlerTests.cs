...
public class ReportHandlerTests
{
    ...
    public async Task CreatePostReport_ShouldSnapshotOwnerAndContent()
    {
        ...
        postRepository.GetByIdAsync(7).Returns(new Post { Id = 7, UserId = 3, User = owner, Content = "  sikayet edilen icerik  ", Status = PostStatus.Published, isPublished = true, isDeleted = false });
        ReportEntity? persisted = null;
        await reportRepository.AddAsync(Arg.Do<ReportEntity>(report => persisted = report));
        var handler = new CreatePostReportCommandHandler(postRepository, reportRepository, unitOfWork, NullLogger<CreatePostReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreatePostReport_OwnPost_ShouldThrowBadRequest()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetByIdAsync(7).Returns(new Post { Id = 7, UserId = 9, Status = PostStatus.Published, isPublished = true });
        var handler = new CreatePostReportCommandHandler(postRepository, reportRepository, unitOfWork, NullLogger<CreatePostReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreatePostReport_AtDailyLimit_ShouldThrowTooManyRequests()
    {
        ...
        var postRepository = Substitute.For<IPostRepository>();
        var reportRepository = Substitute.For<IReportRepository>();
        postRepository.GetByIdAsync(7).Returns(new Post { Id = 7, UserId = 3, Status = PostStatus.Published, isPublished = true });
        reportRepository.CountRecentReportsByUserAsync(9, Arg.Any<DateTime>(), Arg.Any<CancellationToken>()).Returns(3);
        var handler = new CreatePostReportCommandHandler(postRepository, reportRepository, unitOfWork, NullLogger<CreatePostReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions { DailyReportLimit = 3 }));
    ...
    public async Task CreatePostReport_NonPositiveConfiguredLimit_ShouldFallBackToOne()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        postRepository.GetByIdAsync(7).Returns(new Post { Id = 7, UserId = 3, Status = PostStatus.Published, isPublished = true });
        reportRepository.CountRecentReportsByUserAsync(9, Arg.Any<DateTime>(), Arg.Any<CancellationToken>()).Returns(0);
        var handler = new CreatePostReportCommandHandler(postRepository, reportRepository, unitOfWork, NullLogger<CreatePostReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions { DailyReportLimit = 0 }));
    ...
    public async Task CreatePostReport_ExistingPendingReport_ShouldThrowBadRequest()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        postRepository.GetByIdAsync(7).Returns(new Post { Id = 7, UserId = 3, Status = PostStatus.Published, isPublished = true });
        reportRepository.HasPendingPostReportAsync(9, 7, Arg.Any<CancellationToken>()).Returns(true);
        var handler = new CreatePostReportCommandHandler(postRepository, reportRepository, unitOfWork, NullLogger<CreatePostReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateCommentReport_ShouldSnapshotCommentOwnerAndContent()
    {
        ...
        commentRepository.GetVisibleByIdAsync(4, Arg.Any<CancellationToken>()).Returns(new Comment { Id = 4, UserId = 3, User = commentOwner, Content = "yorum icerigi", PostId = 7 });
        ReportEntity? persisted = null;
        await reportRepository.AddAsync(Arg.Do<ReportEntity>(report => persisted = report));
        var handler = new CreateCommentReportCommandHandler(commentRepository, reportRepository, unitOfWork, NullLogger<CreateCommentReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateCommentReport_MissingComment_ShouldThrowNotFound()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        var commentRepository = Substitute.For<ICommentRepository>();
        commentRepository.GetVisibleByIdAsync(4, Arg.Any<CancellationToken>()).Returns((Comment?)null);
        var handler = new CreateCommentReportCommandHandler(commentRepository, reportRepository, unitOfWork, NullLogger<CreateCommentReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateCommentReport_OwnComment_ShouldThrowBadRequest()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        var commentRepository = Substitute.For<ICommentRepository>();
        commentRepository.GetVisibleByIdAsync(4, Arg.Any<CancellationToken>()).Returns(new Comment { Id = 4, UserId = 9, Content = "yorum" });
        var handler = new CreateCommentReportCommandHandler(commentRepository, reportRepository, unitOfWork, NullLogger<CreateCommentReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateCommentReport_ExistingPendingReport_ShouldThrowBadRequest()
    {
        ...
        var commentRepository = Substitute.For<ICommentRepository>();
        var reportRepository = Substitute.For<IReportRepository>();
        commentRepository.GetVisibleByIdAsync(4, Arg.Any<CancellationToken>()).Returns(new Comment { Id = 4, UserId = 3, Content = "yorum" });
        reportRepository.HasPendingCommentReportAsync(9, 4, Arg.Any<CancellationToken>()).Returns(true);
        var handler = new CreateCommentReportCommandHandler(commentRepository, reportRepository, unitOfWork, NullLogger<CreateCommentReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateUserReport_ShouldSnapshotTargetUserAndBio()
    {
        ...
        HandlerTestContext.RegisterUsers(userRepository, target);
        ReportEntity? persisted = null;
        await reportRepository.AddAsync(Arg.Do<ReportEntity>(report => persisted = report));
        var handler = new CreateUserReportCommandHandler(reportRepository, userRepository, unitOfWork, NullLogger<CreateUserReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    public async Task CreateUserReport_AlreadyBannedTarget_ShouldThrowBadRequest()
    {
        ...
        var reportRepository = Substitute.For<IReportRepository>();
        var userRepository = Substitute.For<IUserRepository>();
        HandlerTestContext.RegisterUsers(userRepository, HandlerTestContext.CreateUser(3, "hedef", UserStatus.Banned));
        var handler = new CreateUserReportCommandHandler(reportRepository, userRepository, unitOfWork, NullLogger<CreateUserReportCommandHandler>.Instance, Options.Create(new ReportPolicyOptions()));
    ...
    {
        var reportRepository = Substitute.For<IReportRepository>();
        reportRepository.GetByIdWithDetailsAsync(1).Returns((ReportEntity?)null);
        var handler = new GetReportByIdQueryHandler(reportRepository);
    ...
    {
        var reportRepository = Substitute.For<IReportRepository>();
        reportRepository.GetByIdWithDetailsAsync(1).Returns(new ReportEntity { Id = 1, TargetType = ReportTargetType.Post, TargetPostId = null });
        var handler = new GetReportByIdQueryHandler(reportRepository);
    ...
    public async Task GetReportById_ShouldAggregateRelatedReportsPriorityAndActions()
    {
        ...
        reportRepository.GetByIdWithDetailsAsync(1).Returns(report);
        reportRepository.GetReportsForTargetAsync(ReportTargetType.User, 3, Arg.Any<CancellationToken>()).Returns(relatedReports);
        var handler = new GetReportByIdQueryHandler(reportRepository);
    ...
    public async Task GetReports_ShouldGroupByTargetAndSummarizeReasons()
    {
        ...
        reportRepository.GetFilteredReportGroupsAsync(null, null, null, null, null, 1, 20, Arg.Any<CancellationToken>()).Returns((reports, 3));
        var handler = new GetReportsQueryHandler(reportRepository);
    ...
    public async Task GetReports_ShouldOrderByPriorityThenLastReportDate()
    {
        ...
        reportRepository.GetFilteredReportGroupsAsync(null, null, null, null, null, 1, 20, Arg.Any<CancellationToken>()).Returns((reports, 2));
        var handler = new GetReportsQueryHandler(reportRepository);
    ...
    public async Task GetReports_LongContent_ShouldBeTruncatedIntoPreview()
    {
        ...
        reportRepository.GetFilteredReportGroupsAsync(null, null, null, null, null, 1, 20, Arg.Any<CancellationToken>()).Returns((reports, 1));
        var handler = new GetReportsQueryHandler(reportRepository);
    ...
    public async Task GetReports_ShouldForwardEveryFilterToRepository()
    {
        ...
        reportRepository.GetFilteredReportGroupsAsync(ReportStatus.Pending, ReportTargetType.Comment, ReportReason.Spam, fromDate, toDate, 2, 10, Arg.Any<CancellationToken>())
