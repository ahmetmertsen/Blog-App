...
public class PostHandlerTests
{
    ...
    public async Task CreatePost_ShouldPersistPublishedPostOwnedByCurrentUser()
    {
        ...
        tagRepository.GetByIdsAsync(Arg.Any<List<int>>(), Arg.Any<CancellationToken>()).Returns(new List<Tag>());
        Post? persisted = null;
        await postRepository.AddAsync(Arg.Do<Post>(post => persisted = post));
        var handler = new CreatePostsCommandHandler(postRepository, tagRepository, unitOfWork, NullLogger<CreatePostsCommandHandler>.Instance);
    ...
    public async Task CreatePost_ShouldAttachResolvedTagsAndDeduplicateRequestedIds()
    {
        ...
        tagRepository.GetByIdsAsync(Arg.Do<List<int>>(ids => requestedTagIds = ids), Arg.Any<CancellationToken>()).Returns(tags);
        Post? persisted = null;
        await postRepository.AddAsync(Arg.Do<Post>(post => persisted = post));
        var handler = new CreatePostsCommandHandler(postRepository, tagRepository, unitOfWork, NullLogger<CreatePostsCommandHandler>.Instance);
    ...
    public async Task CreatePost_UnknownTagId_ShouldThrowBadRequestAndNotPersist()
    {
        ...
        var postRepository = Substitute.For<IPostRepository>();
        var tagRepository = Substitute.For<ITagRepository>();
        tagRepository.GetByIdsAsync(Arg.Any<List<int>>(), Arg.Any<CancellationToken>()).Returns(new List<Tag> { new() { Id = 1, Name = "dotnet", NormalizedName = "DOTNET" } });
        var handler = new CreatePostsCommandHandler(postRepository, tagRepository, unitOfWork, NullLogger<CreatePostsCommandHandler>.Instance);
        ...
        await postRepository.DidNotReceiveWithAnyArgs().AddAsync(default!);
    ...
    public async Task UpdatePost_ShouldReplaceContentAndTags()
    {
        ...
        postRepository.GetByIdWithTagsAsync(5).Returns(post);
        var newTag = new Tag { Id = 2, Name = "yeni-tag", NormalizedName = "YENI-TAG" };
        tagRepository.GetByIdsAsync(Arg.Any<List<int>>(), Arg.Any<CancellationToken>()).Returns(new List<Tag> { newTag });
        var handler = new UpdatePostsCommandHandler(postRepository, tagRepository, unitOfWork);
    ...
    public async Task UpdatePost_MissingPost_ShouldThrowNotFound()
    {
        ...
        var tagRepository = Substitute.For<ITagRepository>();
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetByIdWithTagsAsync(5).Returns((Post?)null);
        var handler = new UpdatePostsCommandHandler(postRepository, tagRepository, unitOfWork);
    ...
    public async Task DeletePost_ShouldSoftDeleteAsOwnerDeletion()
    {
        ...
        postRepository.GetByIdWithTagsAsync(5).Returns(post);
        var handler = new DeletePostsCommandHandler(postRepository, unitOfWork, NullLogger<DeletePostsCommandHandler>.Instance);
        ...
        postRepository.Received(1).Update(post);
        await unitOfWork.Received(1).SaveChangesAsync(CancellationToken.None);
    }
    ...
    public async Task GetPostById_ShouldPassAuthenticatedViewerToRepository()
    {
        ...
        postRepository.GetDtoByIdAsync(5, 42, Arg.Any<CancellationToken>()).Returns(dto);
        var handler = new GetPostByIdQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(42));
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetDtoByIdAsync(5, null, Arg.Any<CancellationToken>()).Returns(new PostDto { Id = 5 });
        var handler = new GetPostByIdQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(null));

        var result = await handler.Handle(new GetPostByIdQuery(5), CancellationToken.None);

        Assert.Equal(5, result.Id);
        await postRepository.Received(1).GetDtoByIdAsync(5, null, Arg.Any<CancellationToken>());
    }
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetDtoByIdAsync(Arg.Any<int>(), Arg.Any<int?>(), Arg.Any<CancellationToken>()).Returns((PostDto?)null);
        var handler = new GetPostByIdQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(null));
    ...
    public async Task GetAllPosts_ShouldForwardEveryFilterAndBuildPagedResponse()
    {
        ...
        postRepository.GetPagedAsync(2, 25, 3, 4, "dotnet", "popular", 42, Arg.Any<CancellationToken>()).Returns((items, 51));
        var handler = new GetAllPostsQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(42));
    ...
    public async Task GetPostsByTagId_ShouldForwardViewerAndPaging()
    {
        var postRepository = Substitute.For<IPostRepository>();
        var items = new List<PostDto> { new() { Id = 1 } };
        postRepository.GetPagedByTagIdAsync(3, 1, 20, 42, Arg.Any<CancellationToken>()).Returns((items, 1));
        var handler = new GetAllPostsByTagIdQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(42));
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetPagedByUserIdAsync(7, 1, 20, 42, Arg.Any<CancellationToken>()).Returns((new List<PostDto>(), 0));
        var handler = new GetPostsByUserIdQueryHandler(postRepository, HandlerTestContext.CreateHttpContextAccessor(42));

        await handler.Handle(new GetPostsByUserIdQuery { UserId = 7, Page = 1, Size = 20 }, CancellationToken.None);

        await postRepository.Received(1).GetPagedByUserIdAsync(7, 1, 20, 42, Arg.Any<CancellationToken>());
    }
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetPagedByUserIdAsync(9, 1, 20, 9, Arg.Any<CancellationToken>()).Returns((new List<PostDto>(), 0));
        var handler = new GetMyPostsQueryHandler(postRepository);

        await handler.Handle(new GetMyPostsQuery { UserId = 9, Page = 1, Size = 20 }, CancellationToken.None);

        await postRepository.Received(1).GetPagedByUserIdAsync(9, 1, 20, 9, Arg.Any<CancellationToken>());
    }
    ...
    public async Task GetFollowingPosts_ShouldQueryCurrentUserFeed()
    {
        ...
        postRepository.GetPagedFollowingAsync(9, 2, 10, Arg.Any<CancellationToken>()).Returns((items, 12));
        var handler = new GetFollowingPostsQueryHandler(postRepository);
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetDailyTopPostsAsync(Arg.Any<DateTime>(), Arg.Any<DateTime>(), 50, Arg.Any<CancellationToken>())
            .Returns(new List<TopPostDto> { new() { PostId = 1 }, new() { PostId = 2 }, new() { PostId = 3 } });
        var handler = new GetDailyTopPostsQueryHandler(postRepository, new PassThroughCacheService(), Options.Create(new CacheOptions()));
    ...
    {
        var postRepository = Substitute.For<IPostRepository>();
        postRepository.GetDailyTopPostsAsync(Arg.Any<DateTime>(), Arg.Any<DateTime>(), 50, Arg.Any<CancellationToken>()).Returns(new List<TopPostDto>());
        var cache = new PassThroughCacheService();
        var handler = new GetDailyTopPostsQueryHandler(postRepository, cache, Options.Create(new CacheOptions { DailyTopPostsTtlSeconds = 42 }));
    ...
    public async Task GetDailyTopPosts_CachedResult_ShouldNotHitDatabase()
    {
        ...
        var handler = new GetDailyTopPostsQueryHandler(postRepository, new StubCacheService(cached), Options.Create(new CacheOptions()));

        var result = await handler.Handle(new GetDailyTopPostsQuery(), CancellationToken.None);

        Assert.Same(cached, result);
        await postRepository.DidNotReceiveWithAnyArgs().GetDailyTopPostsAsync(default, default, default, default);
    }
